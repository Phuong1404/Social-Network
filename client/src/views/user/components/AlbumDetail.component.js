...
import { AlbumModal } from '@/views/user/components/AlbumModal.component';
import { toast } from 'react-hot-toast';
import { updateAlbumApi } from '@/views/user/api/updateAlbum.api';
import { PrivacyDropdown } from '@/common/components/Button';
import { dateUtil } from '@/common/utils';
import { MediaModal } from '@/views/user/components/MediaModal.component';
import { deleteAlbumApi } from '@/views/user/api/deleteAlbum.api';
...
export function AlbumDetail({ id, onUpdate, onBack, onDelete }) {
	const { authUser } = useAuth();
	const { data: album, isLoading, mutate, error } = useSWR(`/albums/${id}`, swrFetcher);
	...
	const handleChangePrivacy = async (privacy) => {
...
const ListMedia = ({ id, isOwner, modal }) => {
	const mediaFetcher = useSWRFetcher({ api: `/albums/${id}/medias`, limit: 12 });

	const handleSubmitMedia = async (medias) => {
