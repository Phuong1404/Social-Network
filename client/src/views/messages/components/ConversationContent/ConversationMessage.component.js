...
import { sendMessageApi } from '@/views/messages/api';
import { ConversationAvatar, MessageItem } from '@/views/messages/components';
import { useConversationContext } from '@/views/messages/hooks';
import { conversationConfig } from '@/views/messages/utils';
...
export function ConversationMessage() {
	...
	const msgFetcher = useFetcher({ api: `conversations/${id}/messages` });
	...
	const sendMessage = async (data) => {
		...
		if (data._id) {
			...
			if (data.files)
				msgPlaceholder.media = data.files?.map((file) => ({
					_id: randomUtil.string(24),
					name: file.name,
	...
	useEffect(() => {
		if (id) {
			...
			window.socket.on('typingMessage', ({ senderId }) => {
				const typer = conversation.members.find(
					({ user }) => user._id === senderId && user._id !== authUser?._id
				);
				if (typer) setTypingList((prev) => [...prev, typer]);
			});

			window.socket.on('stopTypingMessage', ({ senderId }) => {
